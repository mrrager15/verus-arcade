  ...
  "src/signer-lite.ts"() {
    ...
    LiteSigner = class {
      constructor(wifKey, verifyNodeUrl) {
        ...
        try {
          const { VerusIdInterface } = __require("verusid-ts-client");
...
var createSha256 = (b) => createHash("sha256").update(b).digest();
...
function createSmartRequire() {
  const strategies = [];
  try {
    strategies.push(createRequire(path2.join(process.cwd(), "noop.js")));
  } catch {
  }
  const entryScript = __require.main?.filename ?? process.argv[1];
  if (entryScript) {
    try {
      const baseDir = path2.dirname(entryScript);
      if (baseDir !== process.cwd()) {
        strategies.push(createRequire(path2.join(baseDir, "noop.js")));
      }
    } catch {
    }
  }
...
function verusAuth(config) {
  ...
  const router = Router();
  router.post("/login", jsonParser({ limit: "4kb" }), async (req, res) => {
  ...
  router.post("/pay-deeplink", jsonParser({ limit: "10kb" }), async (req, res) => {
    const ip = req.ip || req.connection?.remoteAddress || "unknown";
    if (isRateLimited(ip)) {
      return res.status(429).json({ error: "Too many requests" });
    }
  ...
  router.post("/generic-request", jsonParser({ limit: "100kb" }), async (req, res) => {
    const ip = req.ip || req.connection?.remoteAddress || "unknown";
    if (isRateLimited(ip)) {
      return res.status(429).json({ error: "Too many requests" });
    }
  ...
  router.post("/identity-update-request", jsonParser({ limit: "100kb" }), async (req, res) => {
    const ip = req.ip || req.connection?.remoteAddress || "unknown";
    if (isRateLimited(ip)) {
      return res.status(429).json({ error: "Too many requests" });
    }
