...
function verusAuth(config) {
  ...
  if (mode === "daemon") {
    ...
    if (config.chains?.length && config.chains.length > 1) {
      ...
      const defaultChain2 = (config.defaultChain || config.chains[0]).toLowerCase();
      registry = new MultiChainSigner(entries, defaultChain2);
...
// src/cli.ts
var envPath = import_path3.default.join(process.cwd(), ".env");
try {
